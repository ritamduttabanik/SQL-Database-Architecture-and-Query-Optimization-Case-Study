...
-- 1. CLASS FILTERING (2A Students)
SELECT * FROM student
WHERE class = '2A';

-- 2. FILTERING BY CLASS AND GENDER SUBSETS (1B Boys)
SELECT name, mtest
FROM student
WHERE class = '1B' 
  AND sex = 'M';

-- 3. TEXT PATTERN MATCHING / POSITION-SPECIFIC WILDCARDS
-- Isolates names containing the letter "e" as the exact third character
SELECT class, name
FROM student
WHERE name LIKE '__e%';

-- 4. STRIP NOISE STRINGS FROM EXCLUSION LISTS
...
-- 6. TEMPORAL STRING RECONSTRUCTION & ISO-8601 RANGE ANALYSIS
...
-- 7. CONDITIONAL METRIC COUNTING WITH GEOGRAPHIC FILTERS
SELECT COUNT(*) AS number_of_girls
FROM student
WHERE sex = 'F' 
  AND dcode = 'TST';

-- 8. ACADEMIC METRIC PASSING-RATE AGGREGATION
...
-- 9. CATEGORICAL GENDER VOLUME BY CLASSROOM
SELECT class, COUNT(*) AS girl_count
FROM student
WHERE sex = 'F'
GROUP BY class;

-- 10. TEMPORAL STRING PARSING AND ANNUAL VOLUME PARTITIONING
SELECT SUBSTR(dob, 7, 4) AS birth_year, COUNT(*) AS girl_count
FROM student
WHERE sex = 'F'
GROUP BY birth_year;

-- 11. INTUITIVE DATE MATH & DEMOGRAPHIC AGE AVERAGING
SELECT AVG(STRFTIME('%Y', 'now') - SUBSTR(dob, 7, 4)) AS avg_age
FROM student
WHERE class LIKE '1%' 
  AND sex = 'M';

-- 12. MEAN SCORE COMPUTATIONS PARTITIONED BY CLASS
SELECT class, AVG(mtest) AS average_mtest
FROM student
GROUP BY class;

-- 13. MAXIMAL CAP EXTREMUM IDENTIFICATION BY GENDER
SELECT sex, MAX(mtest) AS max_mtest
FROM student
GROUP BY sex;

-- 14. BENCHMARK COMPREHENSIVE DATASET PERFORMANCE AVERAGING
...
-- 15. SET INTERSECTION (Physics & Chemistry Shared Membership)
SELECT ID, FullName
FROM Phy
INTERSECT
SELECT ID, FullName
...
-- 16. COMPLEX SET ALGEBRA (Intersecting Chem/Bio, Excluding Physics)
SELECT ID, FullName
FROM Chem
INTERSECT
SELECT ID, FullName
...
-- 17. ASSET TRACKING & SUPPLY CHAIN SORTING
-- Orders enterprise assembly volumes to optimize storage tracking
SELECT * FROM Client
ORDER BY Qty ASC;

-- 18. CORE KEYWORD DESCRIPTOR SUBSET FILTERING
SELECT * FROM Client
WHERE Descript LIKE '%Shaft%';

-- 19. MULTI-CONDITION PROCUREMENT THRESHOLD EXAMINATION
SELECT * FROM Client
WHERE Qty > 20 
  AND Supplier = 'China Metals Co.';

-- 20. DISTINCT UNIQUE VENDOR IDENTIFICATION
SELECT DISTINCT Supplier
FROM Client;

-- 21. CONDITIONAL DATA MODIFICATION / INVENTORY RE-STOCKING
UPDATE Client
SET Qty = Qty + 10
WHERE Qty < 10;

-- 22. DATA SCRUBBING & INVALID ROW DELETIONS
DELETE FROM Client
WHERE Part_no IN (879, 654, 231, 234);

-- 23. DYNAMIC DDL SCHEMA EVOLUTION (Adding Temporal Attributes)
ALTER TABLE Client
